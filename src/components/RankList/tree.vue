<template>
    <div class="rank-tree">
        ...
        :checked="checked" 
        :sortKey="sortKey" 
        v-bind:isShowhospitalAll="true" 
        v-bind:scrollbarHeight="90" 
        :defaultActiveValue="defaultActiveValue" 
        :checkedKey="checkedKey" 
        :defaultActive="defaultActive" 
        @itemClick="(item) => $emit('itemClick', item)">
            <template v-slot:content="{ item }">
                <ul class="general-tree" :class="`${ftSize('0701', 2)}`">
...
@Component
export default class RankTree extends BaseClass {
     ...
     * sortKey:用于排序的key
     */
    @Prop({ default: 'score' }) readonly sortKey!: string;
    /**
     * checkedKey:选中的key
     */
