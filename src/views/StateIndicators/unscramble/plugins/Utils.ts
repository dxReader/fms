...
export default class Utils {
    ...
    public config = {
        ...
        bg:this.themed('unscramble-node-bg'),
    ...
    //节点下文本显示
    addShape(group: any, type: string, text: string, x: number, y: number, fontColor: string, fontSize: number = 18, fontWeight: number = 400,name:string='',cursor="pointer") {
    ...
    //节点中添加图片
    addShapeImg(group: any, width: number, height: number, x: number, y: number, img: string,name:string='',cursor:string= 'pointer') {
    ...
    addShapeRect(group: any, width: number, height: number, x: number, y: number, fill: string,name:string=''){
        group.addShape('rect', {
            attrs: {
                stroke: '',
                cursor: 'pointer',
    ...
    //将数组拉成一维数组
    oneArray(data: Array<any>): any{
        return [].concat(
            ...data.map((item: any) => {
                if (item.children) {
                    return [].concat(item, ...this.oneArray(item.children));
                }
                return [].concat(item);
            })
        );
