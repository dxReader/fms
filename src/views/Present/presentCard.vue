                ...
                <table border="0" cellspacing="0" cellpadding="0" :class="`${ftSize('0705', 3)}`">
                    ...
                    <template>
                        <tr v-for="(item, key) in config.raelated" :key="key">
                            <td class="name">
                                {{ item.naIndex || '-' }}<i class="sjkp iconfont iconshujuqiapian" @click="showCardFuc(item.idRated, item.naIndex, false, true, item.cdIndex, 2)"></i>
                            </td>
                            <td >
    ...
    private initEchar(obj: any): void{
        ...
        for(let s=0; s<Y.length; s++) {
            if(Y[s] !== null && Y[s] > top) {
                ...
                this.makeSeries(Y[s], Y[s], "bottom", (this as any).common.rgba((this as any).themed('present-card'), .28), (this as any).themed('present-card'), [false, true, true], 1, 1);
            } else {
                this.makeSeries(Y[s], Y[s], "bottom", (this as any).common.rgba((this as any).themed('present-card'), .28), (this as any).themed('present-card'), [false, false, false], null, .01);
            }
        }
        // y轴超过万和多位小数的处理
        ...
        this.$nextTick(()=>{
            this.option = {
                ...
                grid: {
                    top: Math.ceil((this as any).common.getProportion() * 30),
