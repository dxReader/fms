    ...
    <div class="card gl-card">
        <div class="top">
            ...
            <div :class="`name ${ftSize('0309', 2)}`" >
                {{ data.naIndex || '-' }}<i class="sjkp iconfont iconshujuqiapian" @click="showCardFuc(data.cdIndex, data.naIndex, true, false, data.cdIndex, 0)"></i>
            </div>
...
export default class Card extends Vue {
    ...
    private async clickDetail(cdIndex: string){
        let detailData: any = await PresentApi.getRtaDt({ 'cdIndex': cdIndex });
...
.card{
    ...
    @include themify($themes) {
        background-color: rgba(themed('box-bg'), 1);
    }
    border-radius: 4px;
    overflow: hidden;
    float: left;
    &:nth-child(2n+1){
        margin: 0 40px 20px 4px;
    }
    .top{
        position: relative;
        // height: 45px;
