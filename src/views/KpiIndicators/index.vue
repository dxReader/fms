<template>
    <div class="indicators">
        ...
        <div class="dateBox">
            <public-date @dateChange="dateChange" :type="'quarter'" :defaultType="'lastMonth'" />
        </div>
...
import Card1 from "src/views/KpiIndicators/card1.vue";
import Card2 from "src/views/KpiIndicators/card2.vue";
import Card3 from "src/views/KpiIndicators/card3.vue";
...
export default class Indicators extends Vue {
    ...
    private async getCate(){
        this.category = await IndicatorsApi.getCategory({ cdModTp: 25 });
    ...
    private async getUrl(id: string, type: string){
    ...
    private async getLib(cd: string){
        (this as any).list = await IndicatorsApi.getLib({'cdMod': cd, 'cdModTp': 25})
